...
Config.toUnicode = function (emoji) {
    ...
    var unicodeStr = comp.toString("16");
    ...
    if (unicodeStr == 'NaN') {
        ...
        if (emoji.length > 0) {
            surrogate = splitSurrogate(emoji.codePointAt(0));

            if (surrogate)
                return surrogate.replace('\\u', '').toLowerCase();
        }
    }
...
function splitSurrogate(codePoint) {
...
function buildMap()
{
    ...
    var colons = [],codes=[],smileys=[];

    for (var s in Config.smileys) {
        smileys.push(escapeRegExp(s));
    }
    for (var i in Config.emoji_data)
    {
        ...
        Config.rx_smileys = new RegExp('(' + smileys.join('|') + ')', "g");
    }
}
